...
async function cargarReportes() { // Cargar datos para mostrar en los reportes
  try {
    const [mensual, empleados, clientes, stock, top] = await Promise.all([ // Obtener datos de los reportes (varias consultas complejas)
    ...
    // Ventas por mes (CTE, GROUP BY  y HAVING)
    document.getElementById('r-mensual').innerHTML = mensual.length === 0
      ...
      : mensual.map(r => `
        <tr>
          ...
          <td><b>${fmtQ(r.total_mes)}</b></td>
          <td class="txt-muted">${fmtQ(r.promedio_venta)}</td>
        </tr>`).join('');

    // Rendimiento de  empleados (JOIN triple con GROUP BY)
    document.getElementById('r-empleados').innerHTML = empleados.length === 0
      ? '<tr><td colspan="5" class="vacío">Sin datos</td></tr>'
      : empleados.map(e => `
        <tr>
          <td><b>${e.empleado}</b></td>
          <td><span class="badge b-gray">${e.puesto}</span></td>
          <td>${e.total_ventas || 0}</td>
          <td><b>${fmtQ(e.monto_total)}</b></td>
          <td class="txt-muted">${fmtQ(e.promedio_por_venta)}</td>
        </tr>`).join('');

    // para clientes frecuentes (subquery en FROM)
    document.getElementById('r-clientes').innerHTML = clientes.length === 0
      ...
      : clientes.map(c => `
        <tr>
          ...
          <td><b>${fmtQ(c.monto_total)}</b></td>
        </tr>`).join('');

    // stock critico va a ser con exist y subquery en WHERE
    ...
    // Top vendidos  con group by y order by con limit y having 
    document.getElementById('r-top').innerHTML = top.length === 0
      ...
      : top.map(p => `
        <tr>
          <td><b>${p.nombre}</b></td>
          <td>${p.marca}</td>
          <td><b>${p.total_vendido}</b></td>
          <td><b>${fmtQ(p.ingresos_total)}</b></td>
        </tr>`).join('');
