...
// GET — todos pueden ver productos si tienen permiso
...
// POST solo admin y bodeguero pueden crear productos
...
// PUT solo admin y bodeguero pueden editar
  ...
  try {
    const [result] = await pool.query(
      `UPDATE PRODUCTOS SET categoria_id=?, proveedor_id=?, nombre=?, marca=?,
         talla=?, color=?, precio_compra=?, precio_venta=?, stock=?, stock_minimo=?
       WHERE producto_id=?`,
      [categoria_id, proveedor_id, nombre, marca, talla, color,
