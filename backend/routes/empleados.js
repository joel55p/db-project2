...
router.get('/:id', requireAuth, requireRol('admin','supervisor'), async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM EMPLEADOS WHERE empleado_id = ?', [req.params.id]);
...
router.put('/:id', requireAuth, requireRol('admin','supervisor'), async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'UPDATE EMPLEADOS SET nombre=?,puesto=?,email=?,telefono=?,fecha_contratacion=?,salario=? WHERE empleado_id=?',
...
router.delete('/:id', requireAuth, requireRol('admin'), async (req, res) => {
  try {
    const [result] = await pool.query('DELETE FROM EMPLEADOS WHERE empleado_id = ?', [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Empleado no encontrado.' });
